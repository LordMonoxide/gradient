...
public class BlockBronzeOven extends Block implements INamedContainerProvider {
  ...
  @Override
  public TileBronzeOven createTileEntity(final BlockState state, final IBlockReader world) {
  ...
  public Container createMenu(final int id, final PlayerInventory playerInv, final PlayerEntity player) {
    final TileBronzeOven oven = WorldUtils.getTileEntity(player.world, player.getPosition(), TileBronzeOven.class);

    if(oven != null) {
      return new BronzeOvenContainer(id, playerInv, oven);
    }
