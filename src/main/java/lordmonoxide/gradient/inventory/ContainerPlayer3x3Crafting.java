...
public class ContainerPlayer3x3Crafting extends ContainerPlayer {
  ...
  public ContainerPlayer3x3Crafting(final InventoryPlayer playerInventory, final boolean localWorld, final EntityPlayer player) {
    ...
    for(int x = 0; x < 2; ++x) {
      for(int y = 0; y < 2; ++y) {
        this.craftingSlots.add(this.addSlotToContainer(new Slot(this.craftMatrix, y + x * CRAFT_SIZE, 98 + y * 18, 18 + x * 18)));
      }
    }
    ...
    for(int x = 0; x < 3; ++x) {
      for(int y = 0; y < 9; ++y) {
        this.invSlots.add(this.addSlotToContainer(new Slot(playerInventory, y + (x + 1) * 9, 8 + y * 18, 84 + x * 18)));
      }
    }

    for(int i = 0; i < 9; ++i) {
      this.invSlots.add(this.addSlotToContainer(new Slot(playerInventory, i, 8 + i * 18, 142)));
    }
