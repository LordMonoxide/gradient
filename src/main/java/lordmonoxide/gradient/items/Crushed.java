package lordmonoxide.gradient.items;

import lordmonoxide.gradient.GradientMetals;
import net.minecraft.creativetab.CreativeTabs;
import net.minecraft.item.ItemStack;

import java.util.HashMap;
import java.util.Map;

public class Crushed extends GradientItem {
  private static final Map<GradientMetals.Metal, Crushed> items = new HashMap<>();

  public static ItemStack get(final GradientMetals.Metal metal, final int amount) {
    final GradientItem item = items.get(metal);

    if(item == null) {
      return ItemStack.EMPTY;
    }

    return item.getItemStack(amount);
  }

  public final GradientMetals.Metal metal;

  public Crushed(final GradientMetals.Metal metal) {
    super("crushed." + metal.name, CreativeTabs.MATERIALS);
    items.put(metal, this);
    this.metal = metal;
  }
}
