...
public class JournalGather extends JournalComponent {
  ...
  public JournalGather(final String id, final EntityPlayer player, final ItemStack... stacks) {
    super(id);
    this.player = player;
    this.stacks = stacks;
  }
  ...
  @Override
  public void render(final GuiJournalEntry screen) {
    int y = 0;
  ...
  private int hasItem(final ItemStack in) {
    int out = 0;

    for(final ItemStack stack : this.player.inventory.mainInventory) {
