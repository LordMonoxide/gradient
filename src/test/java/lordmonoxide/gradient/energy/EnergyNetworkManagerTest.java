...
public class EnergyNetworkManagerTest {
  ...
  void testIncompatibleDirectionalTransfersDoNotMergeNetworks() {
    ...
    final TileEntity tx     = this.world.addTileEntity(new BlockPos(161, 72, 68), new TileEntityWithCapabilities().addCapability(EnergyNetworkTest.TRANSFER, new TransferNode(), EnumFacing.EAST, EnumFacing.WEST));
    final TileEntity source = this.world.addTileEntity(new BlockPos(162, 72, 68), TileEntityWithCapabilities.source());
    final TileEntity tz     = this.world.addTileEntity(new BlockPos(160, 72, 68), new TileEntityWithCapabilities().addCapability(EnergyNetworkTest.TRANSFER, new TransferNode(), EnumFacing.NORTH, EnumFacing.SOUTH));

    final Map<EnumFacing, EnergyNetwork<IEnergyStorage, IEnergyTransfer>> sinkMap = this.manager.connect(sink.getPos(), sink);
    Assertions.assertEquals(1, sinkMap.size());
    Assertions.assertTrue(sinkMap.containsKey(EnumFacing.SOUTH));
    Assertions.assertEquals(1, this.manager.size());

    final Map<EnumFacing, EnergyNetwork<IEnergyStorage, IEnergyTransfer>> txMap = this.manager.connect(tx.getPos(), tx);
    Assertions.assertEquals(1, txMap.size());
    Assertions.assertTrue(txMap.containsKey(EnumFacing.EAST));
    Assertions.assertEquals(2, this.manager.size());

    final Map<EnumFacing, EnergyNetwork<IEnergyStorage, IEnergyTransfer>> sourceMap = this.manager.connect(source.getPos(), source);
    Assertions.assertEquals(1, sourceMap.size());
    Assertions.assertTrue(sourceMap.containsKey(EnumFacing.WEST));
    Assertions.assertEquals(2, this.manager.size());

    final Map<EnumFacing, EnergyNetwork<IEnergyStorage, IEnergyTransfer>> tzMap = this.manager.connect(tz.getPos(), tz);
